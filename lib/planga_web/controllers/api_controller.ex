    ...
    with                       {:ok, api_key_pair} = Planga.Connection.Persistence.fetch_api_key_pair_by_public_id(public_api_id),
      ...
      call_decrypted(conn, action, params, api_key_pair)
        ...
        _ ->
          conn
        |> put_status(400)
        |> json(%{"status" => 400, "data" => "Bad request; probably missing `action` and `params` keys."})
    end
  end
  ...
  def handle_request(conn, "set_role", params, api_key_pair) do
    ...
    if role == nil || remote_conversation_id == nil || remote_user_id == nil do
      ...
      case Planga.Event.dispatch(
        [:apps, api_key_pair.app_id, :conversations, remote_conversation_id, :users, remote_user_id],
        :set_role,
        %{role: role}
          ) do
